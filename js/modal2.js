	...
	$('#myModalCalck2').on('show.bs.modal', function (event) {
		...
		changePost(type_post, cost_portret);

		//Выбор доставки
		$("#myModalCalck2 input").change(function(e){
	...
	function changePost(type_post, portret_price) {
		localStorage.setItem('type_post', type_post);
		$("#myModalCalck2 #info_post").hide();
		var cost = 0;
		...
		switch (type_post){
			...
			break;
			default:
				$("#myModalCalck2 input[id=fast]").prop("checked", true);
				changePost('fast', portret_price);
		}
